...
describe('Check subcategories list command parameters parser', () => {
  const {
  ...
  test(`${msg1} should be rejected as not valid limit number`, () => {
    const ret = detectCMDSbcListParameters(msg1, user_language, user_time_zone);
  ...
  test(`${msg2} should be rejected as not valid skip number`, () => {
    const ret = detectCMDSbcListParameters(msg2, user_language, user_time_zone);
  ...
  test(`${msg3} should be rejected as not valid skip number`, () => {
    const ret = detectCMDSbcListParameters(msg3, user_language, user_time_zone);
  ...
  it(`"${msg4}" should be rejected as not valid boolean show id flag`, () => {
    const ret = detectCMDSbcListParameters(msg4, user_language, user_time_zone);
  ...
  it(`"${msg5}" should be rejected as not valid boolean show del flag`, () => {
    const ret = detectCMDSbcListParameters(msg5, user_language, user_time_zone);
  ...
  it(`"${msg6}" should be rejected as not valid boolean show arch flag`, () => {
    const ret = detectCMDSbcListParameters(msg6, user_language, user_time_zone);
  ...
  it(`"${msg7}" should be rejected as of duplicated parameter`, () => {
    const ret = detectCMDSbcListParameters(msg7, user_language, user_time_zone);
  ...
  msgs_list_conflict.forEach((msg_list_conflict, idx) => {
    for (const parConflic of msg_list_conflict.expectParConflictName) {
      ...
      it(`"${cmd}" should be rejected as of params conflict`, () => {
        const ret = detectCMDSbcListParameters(
          cmd,
          user_language,
          user_time_zone,
        );
