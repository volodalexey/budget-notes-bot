...
describe('Check undo archive note command handlers', () => {
  ...
  it(`"${cmd_undo_archnt}" should be rejected without arguments`, () =>
    testBotMsgsRes([
      {
        toSendText: cmd_undo_archnt,
  ...
  const cmd_undo_archnt_amb = `${cmd_undo_archnt} a ...`;
  it(`"${cmd_undo_archnt_amb}" should be rejected as of too many arguments`, () =>
    testBotMsgsRes([
      {
        toSendText: cmd_undo_archnt_amb,
        toBeMessage: generateInvInstruction(
          TrKey.INVALID_UNDO_ARCH_NOTE_CMD,
          BotCommandEnum.undoarchnt,
          userLangCode,
  ...
  const cmd_undo_archnt_fake_id = `${cmd_undo_archnt} fakeNoteId`;
  const realCtgTitle = 'realCtgTitle';
  it(`"${cmd_undo_archnt_fake_id}" should be rejected as of errors: not existing note, not visible note for current user`, () =>
    testBotMsgsRes([
      {
      ...
      {
        toSendText: (_) =>
          `${cmd_undo_archnt} ${_[0].preparedData.otherDBNote.id}`,
        toBeMessage: (_) =>
          new ToTranslate(TrKey.NOTE_NOT_EXISTS, userLangCode, {
      ...
      {
        toSendText: (_) =>
          `${cmd_undo_archnt} ${_[0].preparedData.otherDBNote2.id}`,
        toBeMessage: (_) =>
          new ToTranslate(TrKey.NOTE_NOT_EXISTS, userLangCode, {
  ...
  it(`"${cmd_undo_archnt} id" should be resolved as restored from archived note`, () =>
    testBotMsgsRes([
      {
        toSendText: (_, { dbNote1 }) => `${cmd_undo_archnt} ${dbNote1.id}`,
        prepareDB: async () => {
      ...
      {
        toSendText: (_) => `${cmd_undo_archnt} ${_[0].preparedData.dbNote2.id}`,
        toBeMessage: (_) => {
          const ret = _[0].preparedData;
          return new ToTranslate(TrKey.NOTE_TEXT_UNDO_ARCHIVED, userLangCode, {
            id: ret.dbNote2.id,
